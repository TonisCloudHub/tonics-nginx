...
sudo incus exec tonics-nginx -- bash -c "echo 'deb [signed-by=/usr/share/keyrings/nginx-archive-keyring.gpg] http://nginx.org/packages/mainline/debian bookworm nginx' | tee /etc/apt/sources.list.d/nginx.list | tee /etc/apt/sources.list.d/nginx.list"
